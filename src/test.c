...
//     SPREDITclear_canvas(0);
    ...
    */
    ...
    const char* Collider_format = "%s,%s,%s";
    const char* Gravity_format = "%f,%f";
    const char* Jump_format = "%f,%d,%f";
    const char* Movement_format = "%f,%f";
    const char* Sprite_format = "%s,%s,%d";
        ...
        if ('[' == *entity_data)
        {
            ...
            if (*entity_data == ':')
            {
                ...
                if (LSD_Sys_strcmp(component_name,"Body"))
                {
                    ...
                    int init_w;
                    int init_h;
                    sscanf(args,Body_format,&init_x, &init_y,&init_w,&init_h);
                ...
                else if (LSD_Sys_strcmp(component_name,"Clickable"))
                {
                    ...
                    int callback_data;
                    HCS_Clicktype click_type;
                    HCS_Triggertype trigger_type;
                    sscanf(args,Clickable_format,clicktype,triggertype,callback_name,&callback_data);

                    if (LSD_Sys_strcmp(clicktype,"HCS_Click_on"))
                        click_type = HCS_Click_on;
                    else if (LSD_Sys_strcmp(clicktype,"HCS_Click_off"))
                        click_type = HCS_Click_off;
                    else if (LSD_Sys_strcmp(clicktype,"HCS_Click_toggle"))
                        click_type = HCS_Click_toggle;
                    else
                    {
                        #ifdef HCS_DEBUG
                        LSD_Log(LSD_ltERROR,"Enum Name Error");
                        #endif
                    }

                    if (LSD_Sys_strcmp(triggertype,"HCS_Trig_released"))
                        trigger_type = HCS_Trig_released;
                    else if (LSD_Sys_strcmp(triggertype,"HCS_Trig_down"))
                        trigger_type = HCS_Trig_down;
                    else
                    {
                        #ifdef HCS_DEBUG
                        LSD_Log(LSD_ltERROR,"Enum Name Error");
                    ...
                    HCS_Clickable_add(Entity, click_type, trigger_type, callback_name, callback_data);
                } 
                else if (LSD_Sys_strcmp(component_name,"Collider"))  
                {
                    // int HCS_Collider_add(HCS_Entity e, LSD_Vec2f pos_mod, LSD_Vec2i size_mod, char* n);
                    char pos_mod_vec[20];
                    char size_mod_vec[20];
                    char callback_name[20];
                    LSD_Vec2f pos_mod;
                    LSD_Vec2i size_mod;
                    sscanf(args,Collider_format,pos_mod_vec,size_mod_vec,callback_name);

                    sscanf(pos_mod_vec,"Vec(%f,%f)",&pos_mod.x,&pos_mod.y);
                    sscanf(size_mod_vec,"Vec(%d,%d)",&size_mod.x,&size_mod.y);

                    HCS_Collider_add(Entity,pos_mod,size_mod,callback_name);

                }
                else if (LSD_Sys_strcmp(component_name,"Gravity"))
                {
                    double n,m;
                    sscanf(args,Gravity_format,&n,&m);
                    HCS_Gravity_add(Entity,n,m);
                }
                else if (LSD_Sys_strcmp(component_name,"Jump"))
                {
                    double n,l;
                    bool m;
                    sscanf(args,Jump_format,&n,&m,&l);

                    HCS_Jump_add(Entity,n,m,l);
                }
                else if (LSD_Sys_strcmp(component_name,"Movement"))
                {
                    float n,m;
                    sscanf(args,Movement_format,&n,&m);

                    HCS_Movement_add(Entity,n,m);
                }
                else if (LSD_Sys_strcmp(component_name,"Sprite"))
                {
                    // int HCS_Sprite_add(HCS_Entity e, char* n, HCS_Drawtype t, bool use_text);
                    char path[100];
                    char drawtype[20];
                    bool use_text;
                    HCS_Drawtype draw_type;

                    sscanf(args,Sprite_format,&path,&drawtype,&use_text);

                    if (LSD_Sys_strcmp(drawtype,"HCS_Draw_Background0"))
                    {
                        draw_type = HCS_Draw_Background0;
                    }
                    else if (LSD_Sys_strcmp(drawtype,"HCS_Draw_Background1"))
                    {
                        draw_type = HCS_Draw_Background1;   
                    }
                    else if (LSD_Sys_strcmp(drawtype,"HCS_Draw_Background2"))
                    {
                        draw_type = HCS_Draw_Background2;   
                    }
                    else if (LSD_Sys_strcmp(drawtype,"HCS_Draw_Sprite"))
                    {
                        draw_type = HCS_Draw_Sprite;
                    }
                    else if (LSD_Sys_strcmp(drawtype,"HCS_Draw_Decal"))
                    {
                        draw_type = HCS_Draw_Decal;   
                    }
                    else if (LSD_Sys_strcmp(drawtype,"HCS_Draw_Effect"))
                    {
                        draw_type = HCS_Draw_Effect;
                    }
                    else if (LSD_Sys_strcmp(drawtype,"HCS_Draw_Debug"))
                    {
                        draw_type = HCS_Draw_Debug;
                    }
                    else if (LSD_Sys_strcmp(drawtype,"HCS_Draw_Menu0"))
                    {
                        draw_type = HCS_Draw_Menu0;
                    }
                    else if (LSD_Sys_strcmp(drawtype,"HCS_Draw_Menu1"))
                    {
                        draw_type = HCS_Draw_Menu1;
                    }
                    else if (LSD_Sys_strcmp(drawtype,"HCS_Draw_Menu2"))
                    {
                        draw_type = HCS_Draw_Menu2;
                    }
                    else if (LSD_Sys_strcmp(drawtype,"HCS_Draw_DebugUI"))
                    {
                        draw_type = HCS_Draw_DebugUI;
                    }
                    else
                    {
                        #ifdef HCS_DEBUG
                    ...
                    HCS_Sprite_add(Entity,path,draw_type,use_text);
                }
            }
            else if (*entity_data == ';')
            {
