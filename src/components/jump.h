...
int HCS_Jump_add(HCS_Entity e, double n, bool m, double l)
{
    ...
    runData->HCS_Entities[e].comp_ids[HCS_cJump] = LSD_Math_get_id_from_array(runData->HCS_Jump_list, &runData->HCS_Jump_used, HCS_MAX_JUMPS);
    ...
    #ifdef HCS_DEBUG
    LSD_Log(LSD_ltCUSTOM,"HCS: Entity %d mit dem Namen %s wurde erfolgreicht Jump hinzugefügt!",e,HCS_Entity_tag_get(e));
    #endif
...
{
    LSD_Math_remove_object_from_array(runData->HCS_Jump_list,&runData->HCS_Jump_used,&runData->HCS_Entities[e].comp_ids[HCS_cJump]);
    #ifdef HCS_DEBUG
    LSD_Log(LSD_ltCUSTOM,"HCS: Entity %d mit dem Namen %s wurde erfolgreicht Jump entfernt!",e,HCS_Entity_tag_get(e));
    #endif
}
